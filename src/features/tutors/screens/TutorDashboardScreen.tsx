...
export function TutorDashboardScreen() {
  ...
  return <SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.content}>
    <Text style={styles.eyebrow}>TUTOR AREA</Text><Text style={styles.title}>Your learners</Text>
    <View style={styles.metrics}><Metric icon={<UsersRound color={colors.primary} size={24} />} label="Followers" value={followers.length} /><Metric icon={<MessageSquareText color={colors.discovery} size={24} />} label="Open requests" value={requests.filter((item) => item.status === 'open').length} /></View>
    <Text style={styles.sectionTitle}>Assistance requests</Text>
    {requests.length ? requests.map((request) => <View key={request.id} style={styles.card}><Text style={styles.cardTitle}>{request.subject}</Text><Text style={styles.cardBody}>{request.body}</Text><Text style={styles.status}>{request.status.toUpperCase()}</Text><TextInput accessibilityLabel="Reply" multiline onChangeText={setReply} placeholder="Your reply…" style={styles.input} value={reply} /><Pressable accessibilityRole="button" onPress={() => void replyToRequest(request)} style={styles.button}><Text style={styles.buttonText}>Reply</Text></Pressable></View>) : <View style={styles.card}><Text style={styles.cardBody}>No requests are waiting.</Text></View>}
    <Text style={styles.sectionTitle}>Progress comment</Text>
    <View style={styles.card}><TextInput accessibilityLabel="Progress comment" multiline onChangeText={setComment} placeholder="Add a comment for your first followed learner…" style={styles.input} value={comment} /><Pressable accessibilityRole="button" onPress={() => void addComment()} style={styles.button}><Text style={styles.buttonText}>Save comment</Text></Pressable></View>
